...
function itemload(): void {
  ...
  ItemAkusesaripush(12, '敏捷宝石', 6, 0, 2000, 33, '素早さが[0]増加', 150, R1);
  ...
  ItemAkusesaripush(16, '幸运宝石', 4, 0, 2000, 34, '運が[0]増加', 100, R1);
  ...
  ItemAkusesaripush(44, '玩家宝石', 5, 5, 1000000, 35, 'アイテムの完成度に応じて、HP、ATK、DEF、AGIを最高レベルの[0]~[1]％増加させます', 100, R2);
  ...
  ItemAkusesaripush(20, '经验宝石', 0, 0, 5000, 60, '獲得経験値が[0]%増加する', 15, R3);
  ...
  ItemAkusesaripush(109, '燃烧之书', 11, 1, 0, 60, '獲得経験値が[0]%増加する', 150, R4, 3, 0, 73, 84, 92, 93);
  ItemAkusesaripush(64, '勇者宝石', 9, 0, 0, 40, '全てのステータスが[0]増加', 37777, R1);
  ...
  ItemAkusesaripush(66, '战神宝石', 9, 5, 0, 41, '攻撃力防御力素早さが[0]増加', 50000, R1, 1);
  ItemAkusesaripush(67, '戦神の宝石+1', 9, 6, 0, 41, '攻撃力防御力素早さが[0]増加', 144444, R2, 1);
  ItemZyouiGokan([66, 67]);

  ItemAkusesaripush(70, '四神宝珠', 9, 3, 0, 42, 'HP攻撃力防御力素早さが[0]増加', 100000, R2);
  ItemAkusesaripush(71, '四神・宝珠+1', 9, 4, 0, 42, 'HP攻撃力防御力素早さが[0]増加', 400000, R2, 1);
  ItemZyouiGokan([70, 71]);

  ItemAkusesaripush(74, '水晶埃癸斯', 10, 2, 0, 43, 'HP防御力素早さが増加', 9000, R1);
  ItemAkusesaripush(75, 'クリスタルイージス+1', 10, 3, 0, 43, 'HP防御力素早さが増加', 24000, R1, 1);
  ItemZyouiGokan([74, 75]);

  ItemAkusesaripush(24, '属性点水晶', 0, 6, 0, 700, '獲得ステータスポイントが[0]になる', 5, R3);
  ItemAkusesaripush(55, 'ステータスPTクリスタル+1', 10, 6, 0, 701, '獲得ステータスポイントが[0]になる', 6, R3, 1);
  ItemAkusesaripush(113, '神话水晶', 11, 5, 0, 2701, 'Mythic Crystal desc', 100, R3, 3, 0, 24, 55, 71, 114);
  ItemZyouiGokan([24, 55, 113]);

  ItemAkusesaripush(78, '黄昏水晶', 10, 7, 0, 15, 'ステータスポイント40％削減.', 1, R3, 1);
  ItemZyouiGokan([78]);

  ItemAkusesaripush(68, '勇者证明', 10, 0, 0, 820, 'キャラクター能力の効果が[0]%増加', 30, R1, 1);
  ItemAkusesaripush(126, '勇者の証+1', 10, 1, 0, 820, 'キャラクター能力の効果が[0]%増加', 50, R1, 1);
  ItemZyouiGokan([68, 126]);

  ItemAkusesaripush(25, '会心几率戒指', 4, 4, 0, 200, 'クリティカル率が高くなる', 0, R2);
  ItemZyouiGokan([25]);

  ItemAkusesaripush(26, '会心威力戒指', 7, 4, 170000, 210, 'クリティカルダメージが上がる', 0, R3);
  ItemZyouiGokan([26]);

  ItemAkusesaripush(27, '首击会心戒指', 8, 4, 210000, 310, '最初の攻撃は必ずクリティカルになる', 0, R2);
  ItemAkusesaripush(56, '2连击会心戒指', 8, 5, 2000000, 311, '[0]回までの攻撃が必ずクリティカルになる', 0, R3, 1);
  ItemAkusesaripush(57, '3连击会心戒指', 8, 6, 0, 312, '[0]回までの攻撃が必ずクリティカルになる', 0, R3, 1);
  ItemZyouiGokan([27, 56, 57]);

  ItemAkusesaripush(28, '连击率戒指', 4, 5, 0, 250, '連続攻撃の確率が高くなる', 0, R2);
  ItemZyouiGokan([28]);

  ItemAkusesaripush(54, '迷你回复项链', 6, 5, 1700000, 1201, '攻撃時に与えたダメージの[0]%を回復する', 2, R3);
  ItemAkusesaripush(29, '回复项链', 2, 5, 0, 1200, '攻撃時に与えたダメージの[0]%を回復する', 4, R4);
  ...
  ItemAkusesaripush(30, '起死回生项链', 3, 5, 0, 1210, 'HPが0になる攻撃を受けても死なないときがある', 0, R3);
  ...
  ItemAkusesaripush(32, '贪婪戒指', 5, 4, 0, 12, '統計を半分にするドロップ率を二重にする', 0, R1);
  ItemZyouiGokan([32]);

  ItemAkusesaripush(79, '贪欲吊坠', 7, 5, 0, 77, '獲得経験値が0になりました。ドロップ率が40％増加します', 0, R2, 1);
  ItemZyouiGokan([79]);

  ItemAkusesaripush(33, '死亡戒指', 6, 4, 1, 9999, '死', 0, R1, 0);
  ItemZyouiGokan([33]);

  ItemAkusesaripush(76, '回避腰带', 10, 4, 0, 13, '[0]％ゲージ未満では遭遇が無効になります', 45, R3);
  ItemAkusesaripush(77, '回避帯+1', 10, 5, 0, 13, '[0]％ゲージ未満では遭遇が無効になります', 70, R4, 1);
  ItemZyouiGokan([76, 77]);

  ItemAkusesaripush(34, '移动速度宝石', 0, 5, 10000, 100, 'マップ上での移動速度が少し上がる', 60, R2);
  ItemAkusesaripush(35, '移動速度宝石+1', 1, 5, 200000, 100, 'マップ上での移動速度が上がる', 100, R2);
  ItemZyouiGokan([34, 35]);

  ItemAkusesaripush(36, '剩余战斗戒指', 1, 6, 10000, 500, '装備中残り戦闘が[0]増える', 1, R3);
  ...
  ItemAkusesaripush(80, '异世界碎片', 9, 7, 0, 40, '全てのステータスが[0]増加', 500000, R2);
  ItemAkusesaripush(90, '別の世界の断片+1', 9, 8, 0, 40, '全てのステータスが[0]増加', 1000000, R2, 1);
  ItemZyouiGokan([80, 90]);

  ItemAkusesaripush(81, '保护之石', 2, 2, 0, 1111, '敵から受けるダメージ[0]%減少する + HP[1]増加する', 20, R1, 0, 1);
  ItemZyouiGokan([81]);

  ItemAkusesaripush(82, '力量之石', 7, 2, 0, 2222, '敵に与えるダメージが[0]%増加する + ATKが[1]増加する', 20, R1, 0, 1);
  ItemZyouiGokan([82]);

  ItemAkusesaripush(84, '天使之羽', 2, 10, 0, 3333, 'HP[0]%増加する + EXF[1]%増加する', 105, R4, 0, 2);
  ItemZyouiGokan([84]);

  ItemAkusesaripush(85, '不灭之力', 0, 13, 0, 4000, 'HPが0になると[0]%で復活します。復活するたびに、全体能力値が[1]%上がります。', 2, R4, 0, 2);
  ItemZyouiGokan([85]);

  ItemAkusesaripush(86, '风暴之力', 3, 13, 0, 4001, '連続攻撃確率が[0]％上昇し、連続攻撃時のダメージが[1]％増加します。', 50, R4, 0, 2);
  ItemZyouiGokan([86]);

  ItemAkusesaripush(87, '天空之力', 2, 13, 0, 4002, '自分のレベルx[0]分のダメージを追加します。', 50, R4, 0, 2);
  ItemZyouiGokan([87]);

  ItemAkusesaripush(88, '大地之力', 1, 13, 0, 4003, '敵から受けるダメージ[0]％減少 + HP[1]％増加。', 0, R1, 0, 2);
  ItemZyouiGokan([88]);

  ItemAkusesaripush(89, '加速沙漏', 6, 4, 0, 4100, 'EXP[0]％の増加、バトルポイント[1]倍消費経験値獲得率[1]倍に増加。', 0.5, R3, 1, 2);
  ItemAkusesaripush(127, '加速沙漏+1', 9, 4, 0, 4101, 'EXP[0]％の増加、バトルポイント[1]倍消費経験値獲得率[1]倍に増加。', 0.5, R3, 1, 2);
